...
<body>
  ...
  <main class="shell">
    ...
    <section class="panel main" aria-label="chat">
      <header class="topbar">
        ...
        <div class="actions">
          <button class="icon-btn" id="wakeBtn" title="Enable 'Hey MOSES' wake word" aria-label="Enable 'Hey MOSES' wake word">WAKE</button>
    ...
    <aside class="panel intel">
      ...
      <nav class="tabs" aria-label="local state tabs">
        ...
        <button data-tab="ar_tags">AR</button>
  ...
  <script>
    const state = {
      ...
      visionMode: "none",
      visionStream: null,
      data: { memory: [], todos: [], reminders: [], notes: [], ar_tags: [], pending_changes: [], model: "" }
    };
    ...
    const el = {
      ...
      visionAR: document.getElementById("visionAR"),
    ...
    function toggleWake() {
      if (state.waking) {
        stopWake();
        addMessage("system", "Wake word detection disabled.");
      } else {
        startWake();
        addMessage("system", "Wake word detection enabled. Say 'Hey MOSES' to command.");
      }
    }
    ...
    function startWake() {
      ...
      if (!Recognition) {
        addMessage("system", "Wake word is not supported in this browser.");
        return;
      }
      ...
      wakeRecognition.onresult = (event) => {
        ...
        for (let i = event.resultIndex; i < event.results.length; ++i) {
          if (event.results[i].isFinal) {
            ...
            const trigger = ["hey moses", "hi moses", "okay moses", "yo moses", "moses"].find(t => transcript.includes(t));
            
            if (trigger) {
              const command = transcript.split(trigger)[1].trim();
              if (command) {
                // Detected wake word + command in one go
                el.prompt.value = command;
                if (state.visionMode !== "none") sendVisionPrompt(command);
                else sendPrompt(command);
              } else {
                // Just wake word, trigger listening mode
                speak("Yes?");
                startVoice();
              }
              break;
      ...
      wakeRecognition.onerror = (e) => {
        if (e.error !== "no-speech") console.warn("Wake Error:", e.error);
      };

      wakeRecognition.onend = () => {
        if (state.waking && !state.busy) {
    ...
    function stopWake() {
      state.waking = false;
      el.wakeBtn.classList.remove("active");
      if (wakeRecognition) {
    ...
    function startVoice() {
      ...
      if (state.waking && wakeRecognition) {
        wakeRecognition.stop(); // Temporarily stop wake to avoid conflict
      }
      ...
      recognition.onend = () => {
        ...
        if (state.waking) startWake(); // Resume wake word after voice command
      };
      recognition.start();
    }
    ...
    function renderState() {
      ...
      const records = {
        ...
        brain: Object.entries(state.data.brain_map || {}).map(([tag, ids]) => ({
          label: `#${tag}`,
          description: `${ids.length} related items`,
          id: `tag-${tag}`
        })),
        ar_tags: state.data.ar_tags || [],
    ...
    function renderRecord(record) {
      ...
      title.textContent = record.label || record.task || record.text || record.title || record.key || record.path || record.id;
      node.appendChild(title);

      if (record.value || record.body || record.description) {
        const body = document.createElement("div");
        body.className = "item-meta";
        body.textContent = record.value || record.body || record.description;
      ...
      const bits = [record.id, record.action, record.priority, record.due, record.remind_at, record.status,
                    record.x !== undefined ? `x: ${record.x}` : null,
                    record.y !== undefined ? `y: ${record.y}` : null].filter(Boolean);
    ...
    function stopVision() {
      ...
      if (el.visionAR) {
        const ctx = el.visionAR.getContext("2d");
        ctx.clearRect(0, 0, el.visionAR.width, el.visionAR.height);
      }
    }
